#!/usr/bin/env python3
"""Scaffold a new GRC atom from the standard template.
...
def main(argv: list[str]) -> int:
    ...
    # Copy template tree and fill in atom name
    ...
    # Add stub entry to atoms.json registry
    if REGISTRY.exists():
        ...
        reg["atoms"].append({
            ...
            "script": "REPLACE — path to backing engine script",
            "input_schema": {},
            "output_schema": {"human_review_required": "boolean"},
            "tags": [],
        })
        REGISTRY.write_text(json.dumps(reg, indent=2) + "\n", encoding="utf-8")
        print(f"added stub entry to {REGISTRY.relative_to(ROOT)}")

    loc = dest.relative_to(ROOT)
    print(f"created {loc}")
    print(f"next: edit SKILL.md + MAINTAINER.md + evals/evals.json (replace placeholders),")
    print(f"      update atoms.json entry (script, input_schema, output_schema, tags),")
    print(f"      then run: python3 tools/sync_check.py")
    return 0
